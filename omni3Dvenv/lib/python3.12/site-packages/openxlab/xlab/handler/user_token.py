...
def update_package(use_official_index=False):  
    ...
    Raises exceptions:  
        - An exception is issued if the package upgrade fails due to a subprocess error (e.g., network issues, invalid package name, etc.).  
    ...
    try:  
        ...
        raise EnvironmentError(f"An error occurred while installing or upgrading 'openxlab' using pip: {e}")

    except Exception as e:  
        raise Exception(f"An unknown exception occurred while installing or upgrading 'openxlab' using pip: {e}")
...
def get_version_cache_expiration(last_update_time:datetime)->datetime:
    ...
    Returns:
        datetime: The calculated expiration time for the cache, which will be one day later than the last update time, and indicate when the cache becomes stale and needs refreshing again.
    """
...
def trigger_update_check():
    ...
    try:
        ...
        if not is_latest_version:
            print(
                f"Warning: you are using openxlab version {current_version}; however, version {latest_version} is available. Consider using 'pip install -U openxlab' to avoid version compatibility problems."
            )
...
def update_version_check():
    ...
    try:
        ...
        latest_version_obj = LatestVersion(is_latest_version=is_latest_version,latest_version=latest_version,auto_update=auto_update)
...
class UserVersion(object):
    ...
    def store_to_local(self):
        ...
        # blocking=False means non-blocking, it is prior than timeout. Set timeout=-1 can make update check and write version.json as a atomic operation
